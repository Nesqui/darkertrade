...
const paths = computed(() => ({
  currentUser: {
    existingItem: `/user/${userStore.currentUser.nickname}/items/${selectedChat.value.chat?.bid.existingItem.id}`,
    suggestedItem: `/user/${userStore.currentUser.nickname}/items/${selectedChat.value.chat?.bid.suggestedExistingItemId}`
  },
  conversationContact: {
    existingItem: `/user/${conversationContact.value?.nickname}/items/${selectedChat.value.chat?.bid.existingItem.id}`,
    suggestedItem: `/user/${conversationContact.value?.nickname}/items/${selectedChat.value.chat?.bid.suggestedExistingItemId}`
  }
}))
</script>
    ...
    <div class="selected-chat__info">
      <div class="selected-chat__info__li">
        ...
        <div class="bid-actions">
          <el-button
            ...
            :class="{
              'icon-active': route.path === paths.currentUser.existingItem
            }"
            @click.stop="push(paths.currentUser.existingItem)"
            circle
          ...
          <el-button
            v-else
            :class="{
              'icon-active': route.path === paths.conversationContact.existingItem
            }"
            @click.stop="push(paths.conversationContact.existingItem)"
            circle
            ><el-icon> <View /> </el-icon
          ></el-button>
          <el-tooltip
      ...
      <div v-if="selectedChat.chat?.bid?.suggestedExistingItemId" class="selected-chat__info__li">
        ...
        <el-button
          v-if="chatStore.currentChatOfferType !== 'receivedOffers'"
          :class="{
            'icon-active': route.path === paths.currentUser.suggestedItem
          }"
          @click.stop="push(paths.currentUser.suggestedItem)"
          circle
          ><el-icon> <View /> </el-icon
        ></el-button>
        <el-button
          v-else
          :class="{
            'icon-active': route.path === paths.conversationContact.suggestedItem
          }"
          @click.stop="push(paths.conversationContact.suggestedItem)"
          circle
          ><el-icon> <View /> </el-icon
        ></el-button>
      </div>
