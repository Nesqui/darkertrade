...
const emit = defineEmits(['textRecognitionFinished', 'itemRecognitionFinished', 'statRecognitionFinished', 'startRecognition'])
...
const statRecognition = (text: string, item?: Item) => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (numericValue !== null) {
      const matchingAttribute = possibleAttributes.find(attr => line.toLowerCase().includes(attr.name.toLowerCase()));
...
const textRecognition = async (file: any) => {
  ...
  const { data: { text } } = await worker.recognize(file.raw || file);
...
<template>
  <div class="img-recognition">
    <el-upload v-loading="loading" :limit="1" :disabled="loading" :on-exceed="handleExceed" v-model:file-list="fileList"
      class="upload-wrapper" action="" :auto-upload="false">
      ...
      <template #tip>
        <div class="upload-description">
          <div class="pr-2">
            ...
            <p>Simply paste an image from your clipboard to identify items using <strong>CTRL+V</strong>. Ensure clear
              images for accurate results.</p>
            ...
            <p>Feel free to paste new images at any time. Our tool will promptly recognize and process each image for your
              convenience.</p>
          </div>
          <div>
