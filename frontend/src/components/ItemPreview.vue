...
const divineItems: DivineItems = {
  'Heater Shield':
    'Can deflect projectile spells while in defensive stance. After the reflection is triggered, you remain able to reflect for 3 seconds, and cannot reflect again for 20 seconds after performing a spell reflection.',
  'Survival Bow':
    'When shooting a bow, the action speed is very fast and the arrow flies in a straight line.',
  Halberd: 'Hitting an enemy deals 10 additional magic damage and burns them for 3 seconds.',
  'Horsemans Axe':
    'Hitting the target will render them unable to receive any healing effects for 20 seconds.',
  ...
  'Double Axe':
    'When receiving a lethal attack, HP becomes 1 and becomes invincible for 2 seconds. This effect can only be triggered once during an adventure.',
  ...
  'Morning Star':
    "Crushes the enemy's armor with each hit, reducing the target's physical defense by -25%. Stacks up to 3 times.",
  Rapier:
    'Hitting an enemy deals 2 additional magic damage, freezes them, and frostbites them for 10 seconds.'
}
    ...
    <div class="tat-frame" :class="{ 'no-hover': noHover }">
      <div class="offer-header">
        <div class="text-divider">
          ...
          <!-- CLASSIC NAME  -->
          <div v-else>
            <span :class="`darker-title ${wantedPrice ? 'rarity-' + rarity : ''}`">
              {{ item?.name || 'Choose item' }}</span
            >
          </div>
        </div>
        ...
        <div v-if="offerType" class="offer-header__item">
          ...
          <p class="offer-header__small">
            {{ moment.fromNow(updatedAt || new Date().toString()) }}
          </p>
        </div>
        <div v-if="offerType" class="offer-header__item">
      ...
      <div class="item-description">
        ...
        <div class="stats" v-if="baseStats.length">
          ...
          <span
            v-for="(stat, index) in baseStats.filter((stat) => stat.isBase)"
            :key="index"
            class="rarity-0 base-stat"
          >
            {{ stat.value > 0 ? `+${stat.value}` : stat.value
            }}{{ getAttributeSymbolById(stat.attributeId) }}
            {{ truncate(getAttributeNameById(stat.attributeId), 35) }}
          </span>
        </div>

        <div class="stats" v-if="additionalStats.length">
        ...
        <div
          v-if="
            item?.name &&
            stats?.filter((stat) => !stat.isBase).length === 5 &&
            getDivineItem(item.name)
          "
...
$frameHeight: 160px;
$item-description-padding: 0.7rem 0.5rem;
